...
public class MapboxActivity extends AppCompatActivity implements OnNavigationReadyCallback, NavigationListener,
        ProgressChangeListener, InstructionListListener, SpeechAnnouncementListener, BannerInstructionsListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        double latFrom = intent.getExtras().getDoubleArray("latLongs")[0];
        double lonFrom = intent.getExtras().getDoubleArray("latLongs")[1];
        double latTo = intent.getExtras().getDoubleArray("latLongs")[2];
        double lonTo = intent.getExtras().getDoubleArray("latLongs")[3];

        ORIGIN = Point.fromLngLat(lonFrom, latFrom);
        DESTINATION = Point.fromLngLat(lonTo, latTo);

        CameraPosition initialPosition = new CameraPosition.Builder()
                .target(new LatLng(ORIGIN.latitude(), ORIGIN.longitude())).zoom(INITIAL_ZOOM).build();
    ...
    private void startNavigation(DirectionsRoute directionsRoute) {
        NavigationViewOptions.Builder options = NavigationViewOptions.builder().navigationListener(this)
                .directionsRoute(directionsRoute).shouldSimulateRoute(false).progressChangeListener(this)
                .instructionListListener(this).speechAnnouncementListener(this).bannerInstructionsListener(this);
    ...
    private void fetchRoute() {
        NavigationRoute.builder(this).accessToken(Mapbox.getAccessToken()).origin(ORIGIN).destination(DESTINATION)
                .alternatives(true).build().getRoute(new Callback<DirectionsResponse>() {
                    @Override
                    ...
                    public void onFailure(Call<DirectionsResponse> call, Throwable t) {
                        Log.d("RRRR", "falhou get Route");
                    }
                });
    }

    /**
     * Sets the anchor of the spacer for the speed widget, thus setting the anchor
     * for the speed widget (The speed widget is anchored to the spacer, which is
     * there because padding between items and their anchors in CoordinatorLayouts
     * is finicky.
     *
    ...
    private void setSpeed(Location location) {
        ...
        spannableString.setSpan(new AbsoluteSizeSpan(mphTextSize), string.length() - 4, string.length(),
                Spanned.SPAN_INCLUSIVE_INCLUSIVE);

        spannableString.setSpan(new AbsoluteSizeSpan(speedTextSize), 0, string.length() - 3,
                Spanned.SPAN_INCLUSIVE_INCLUSIVE);
